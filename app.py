...
def todos_list():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            todo = form.data
            todos.create_task(todo)
        return redirect(url_for("todos_list"))

    return render_template("todos.html", form=form, todos=todos.show_all_tasks(), error=error)


@app.route("/todos/<int:todo_id>/", methods=["GET", "POST"])
def todo_details(todo_id):
    todo = todos.get_task(todo_id)
    if not todo:
        abort(404)
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            newid = todo_id
            todo['id'] = newid
            todos.update_task(todo_id, todo)
        return redirect(url_for("todos_list"))
...
def todos_list_api_v1():
    return jsonify(todos.show_all_tasks())
...
def delete_todo(todo_id):
    if not todos.get(todo_id):
        abort(404)
    result = todos.delete_task(todo_id)
